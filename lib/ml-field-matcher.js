...
const trainingData = [
  ...
  { label: "BE-BTech %", target: "aggregateCGPA" },
  { label: "BE-BTech % (till recent result declared)*", target: "aggregateCGPA" },
  { label: "%  BE-BTech (till recent result declared)*", target: "aggregateCGPA" },
  { label: "% BE-BTech (till recent result declared)*", target: "aggregateCGPA" },
  { label: "% BE-BTech", target: "aggregateCGPA" },
  { label: "BE-BTech Percentage", target: "aggregateCGPA" },
  { label: "BTech %", target: "aggregateCGPA" },
  { label: "BE %", target: "aggregateCGPA" },
  { label: "Engineering Percentage", target: "aggregateCGPA" },
  ...
  { label: "Course*", target: "course" },
  { label: "Current Course", target: "course" },
  { label: "Your Course", target: "course" },
  ...
  { label: "Name of Course", target: "course" },
  { label: "Course Name*", target: "course" },
...
tokenize(text) {
  ...
  keywordGroupMatch(fieldLabel) {
    ...
    const fieldKeywords = {
      ...
      'collegeName': {
        keywords: ['college', 'graduation', 'name', 'college name', 'graduation college', 
             'graduation college name', 'full name', 'mention', 'full', 
      ...
      'course': {
        keywords: ['course', 'degree', 'program', 'programme', 'current course', 'your course', 'name of course', 'course name'],
        required: ['course', 'degree'],
        boost: ['current course', 'name of course', 'course name']
      },
      'degree': {
        keywords: ['degree', 'program', 'programme'],
        required: ['degree']
      },
      'specialization': {
      ...
      'twelfthCollegeName': {
        ...
        required: ['12th', 'twelfth', 'hsc', 'college'],
        boost: ['name']
      },
      'twelfthPassingYear': {
      ...
      // College Email
      'collegeEmailId': {
        keywords: ['college', 'email', 'id', 'college email', 'college email id', 'institutional email', 'university email'],
...
keywordBasedMatch(fieldLabel) {
  ...
  // ... rest of the function remains the same
    ...
    if (/\b(course|branch|department|dept|stream|specialization)\b/.test(text)) {
      // Check for "Current Course" or "Name of Course" patterns first
      if (/\b(current\s+course|your\s+course|name\s+of\s+course|course\s+name)\b/.test(text)) {
        return { predictedField: 'course', confidence: 0.98 };
      }
...
function matchField(fieldLabel) {
  ...
  // âœ… Try smart extraction first using cleaned label
  const smartResult = extractFieldFromComplexLabel(cleaned);
  if (smartResult && smartResult.field) {
    console.log('[matchField] Smart extraction result:', smartResult.field);
    return {
  ...
  return mlMatcher.predict(cleaned);
}
