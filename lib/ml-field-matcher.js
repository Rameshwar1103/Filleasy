...
const trainingData = [
  ...
  { label: "Address", target: "currentAddress" },
  { label: "current Address", target: "currentAddress" },
  { label: "Address Line 1", target: "currentAddress" },
...
class MLFieldMatcher {
  ...
  static preprocessFieldLabel(label) {
    ...
    const fillerWords = ['the', 'of', 'your', 'enter', 'please', 'provide', 'mention', 'if', 'till', 'recent', 'result', 'declared','current'];
...
// BRANCH FIELD (merged branch + specialization)
    ...
    'branch','department','dept','stream','specialization','specialisation'],
      ...
      'certificationLink': {
        keywords: ['certification', 'certificate', 'link', 'certification link', 'certificate link'],
        required: ['certification', 'certificate', 'link']
      },
      // CODING PLATFORM FIELDS
      ...
      'currentAddress': {
        keywords: ['current', 'address', 'current address'],
      ...
      'permanentAddress': {
        keywords: ['permanent', 'address' , 'permanent address'],
        required: ['permanent', 'address' ]
      },
  ...
  // ... rest of the function remains the same
    ...
    if (/\b(address|addr)\b/.test(text)) {
      ...
      if (/\b(current|present|now)\b/.test(text)) {
        return { predictedField: 'currentAddress', confidence: 0.90 }; // Current address
      }
      if (/\b(permanent|permanent\s+address)\b/.test(text)) {
        return { predictedField: 'permanentAddress', confidence: 0.90 }; // Permanent address
      }
      // Default to street address
      return { predictedField: 'currentAddress', confidence: 0.85 };
    }
    ...
    if(/\b(certificate|certification|certifications)\b/.test(text)) {
      return { predictedField: 'certificationLink', confidence: 0.95 };
    }
